...
# --- UI HELPER FUNCTIONS ---
# These functions use `st` commands, so they must stay in the main app file.
def display_infographics(images, txt):
    ...
    for item in images:
        if os.path.exists(item["path"]):
            ...
            if width:
                st.image(image, width=width, caption=f"*{caption_text}*")
            else:
...
# --- 2. SESSION STATE INITIALIZATION FOR THE FORM PATTERN ---
...
with st.sidebar:
    ...
    # Use pending_lang for instant UI feedback in the sidebar itself
    ...
    def format_ward_options(ward):
        return f"Ward {ward}" if st.session_state.pending_lang == 'en' else f"{current_txt['select_ward']} {to_nepali_num(ward)}"
    ...
    if st.session_state.pending_ward:
        # We need to load data here just to populate the multiselect options
        ...
        if df_points is not None and not df_points.empty:
            with st.expander(current_txt['map_layers'], expanded=True):
                def format_category_labels(cat):
                    return current_txt.get('map_categories', {}).get(cat, cat)
                st.multiselect(
                    current_txt['select_points'],
                    options=sorted(df_points['Category'].unique()),
                    format_func=format_category_labels,
                    key='pending_map_categories',
                    placeholder=current_txt['choose_options']
                )
...
if not st.session_state.active_ward:
    ...
    active_map_categories = st.session_state.active_map_categories
    ...
    with col2:
        ...
        for i, sector_name_en in enumerate(ALL_SECTORS):
            with sector_tabs[i]:
                if sector_name_en in sectors:
                    display_sector_content(sectors[sector_name_en], txt)
                else:
